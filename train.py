...
"""
# USAGE
# python train.py --dataset dataset --model plantgrowth.model --labelbin label_bin_plantgrowth.pickle
...
from sklearn.preprocessing import LabelBinarizer                   #allows input of class labels,transform our class labels into one-hot encoded vectors 

#from pyimagesearch.datasets import simpledatasetloader
#from pyimagesearch.preprocessing import aspectawareprocessor 
#from pyimagesearch.preprocessing import imagetoarrayprocessor

#from keras.utils.np_utils import to_categorical                    #allow us to take an integer class label prediction from our Keras CNN and transform it back into a human-readable label 
...
for imagePath in imagePaths:
    ...
    #aap = AspectAwareProcess(96,96)
    #iap = ImageToArrayProcessor()
    #sdl = SimpleDatasetLoader(processors=[aap, iap])
...
#data augmentation increases the generalizability of the model 
...
#trainY = to_categorical(trainY) #not required
#testY = to_categorical(testY)  #not required 

#do not apply data augmentation at testing time 
#apply the augmentation only at training time
#ie. random geometric transformations (translations, rotations, scaling, shearing, horizontal flipping) 
...
#weights = model.get_weights() #not required
#model.set_weights(weights)    #not required
